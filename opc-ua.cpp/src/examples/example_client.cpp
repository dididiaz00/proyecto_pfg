...
int main(int argc, char ** argv)
{
  ...
  try
    {
      ...
      OpcUa::Node mystr;
      OpcUa::Node mybool;
      //std::string newString = "Que tal?";
      
      //OpcUa::Variant newString("Que tal?", OpcUa::VariantType::String);
      /*OpcUa::Variant newString;
      newString.SetString("Que tal?");

      OpcUa::WriteValue writeValue;
      writeValue.NodeId = mystr.GetId();
      writeValue.AttributeId = OpcUa::AttributeId::Value; // Especifica que estamos escribiendo el valor.
      writeValue.Value = newString;

      client.WriteAttributes({writeValue});
      logger->info("Valor de Node {} ha sido escrito a: {}", mystr, newString.ToString());*/
      ...
      std::vector<std::string> varpath({ std::to_string(idx) + ":NewObject", "MyString" });
      ...
      logger->info("Got sub handle: {}, sleeping 180 seconds", handle);
  ...
  return -1; //Que significa el -1 o el 0
}
