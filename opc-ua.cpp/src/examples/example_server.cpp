...
void RunServer()
{
  ...
  Node myvar = newobject.AddVariable(idx, "MyVariable", Variant(0));
  Node myprop = newobject.AddVariable(idx, "MyProperty", Variant(8.8));
  Node mymethod = newobject.AddMethod(idx, "MyMethod", MyMethod);
  Node mystr = newobject.AddVariable(idx, "MyString", "Hola, mundo");
  Node mybool = newobject.AddVariable(idx, "MyBool", Variant(false));
  ...
  SubClient clt;
  std::shared_ptr<Subscription> sub = server.CreateSubscription(100, clt);
  //std::unique_ptr<OpcUa::Subscription> uniqueSub = std::move(sharedSub);
